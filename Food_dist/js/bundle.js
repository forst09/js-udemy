...
function cards() {
  ...
  if (cardsContainer) {
    var Card = /*#__PURE__*/function () {
      function Card(imgSrc, imgAlt, subtitle, descr, cost) {
...
function slider(_ref) {
  ...
  var sliderNav = slider.querySelector(totalCounter);
  var sliderNext = sliderNav.querySelector(nextArrow);
  var sliderPrev = sliderNav.querySelector(prevArrow);
  var slidesWrapper = slider.querySelector(wrapper);
  var slidesField = slider.querySelector(field);
  ...
  function findSlide(slideIndex) {
    ...
    document.querySelector(".dot[slideindex=\"".concat(slideIndex, "\"]")).style.opacity = '1';
  }

  //write current index
  function insertIndex() {
    if (sliderIndex < 10) {
      sliderNav.querySelector('#current').textContent = "0".concat(sliderIndex);
    } else {
  ...
  var _loop = function _loop() {
    var paginationDot = document.createElement('button');
    ...
    paginationDot.addEventListener('click', function () {
      var index = paginationDot.getAttribute('slideIndex'),
        differenceIndex;
  ...
  findSlide(sliderIndex);
  if (slides.length < 10) {
    sliderNav.querySelector('#total').textContent = "0".concat(slides.length);
  } else {
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    ...
    if (t <= 0) {
      ...
      days = Math.floor(t / (1000 * 60 * 60 * 24)), hours = Math.floor(t / (100 * 60 * 60) % 24), minutes = Math.floor(t / 1000 / 60 % 60), seconds = Math.floor(t / 1000 % 60);
    }
    return {
      'total': t,
      'days': days,
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2024-06-11');
